...
# CONFIGURABLE: set this to the line number in the CSV you want to read from. Usually (row in results CSV + 1).
# NOTE! it is 1-indexed like everything in R.
CHECKPOINT_ROW <- 999999
# CONFIGURABLE: change this to the file you want to read from
FILENAME <-  "reviews-1-115.csv"
...
# read existing checkpoint'd information as well because it's important to analyze the user's existing genres
result <- read.csv("../reviews_market_basket_data.csv")
...
for (name in list.files()){
  if(name == FILENAME) {
    ...
    for(row in CHECKPOINT_ROW:nrow(temp)) {
