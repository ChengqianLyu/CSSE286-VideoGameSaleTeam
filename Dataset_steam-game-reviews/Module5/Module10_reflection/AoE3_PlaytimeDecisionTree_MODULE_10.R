...
# =================
# Module 10-specific
aoe3_pred_boost_PREDICTED_PROB <- predict(aoe3_model_boost10, aoe3_test, type="prob")
...
# add on the prediction columns to a new data frame (the original model results are a factor,
# which is probably not the best to manipulate into a data frame, since those results can
# be used for other things)
# both aoe3_pred_boost10 and aoe3_test$playtime_above_median should be factors for caret
aoe3_pred_boost10_caret <- data.frame(predict_type = aoe3_pred_boost10, actual_type = aoe3_test$playtime_above_median)

library(caret)
# for the purposes of discussion, assume >= is positive
confusionMatrix(aoe3_pred_boost10_caret$predict_type, aoe3_pred_boost10_caret$actual_type, positive = ">= Mdn PT")

# for the ROC curve, we need prediction probabilities (for sake of discussion, < is negative)
aoe3_pred_boost10_caret$prob_lt_mdn <- aoe3_pred_boost_PREDICTED_PROB[,1]
library(pROC)
aoe3_roc <- roc(aoe3_pred_boost10_caret$actual_type, aoe3_pred_boost10_caret$prob_lt_mdn)
plot(aoe3_roc, main = "ROC curve for Playtime at Review from Other Factors", col = "blue", lwd = 2, legacy.axes = TRUE)
auc(aoe3_roc)

# k-fold CV
# aoe3_data$playtime_above_median is the predictor variable
folds <- createFolds(aoe3_data$playtime_above_median, k=10)
# note that the random seed was set earlier
str(folds)
library(irr)
cv_results <- lapply(folds, function(x) {
  aoe3_train_temp <- aoe3_data[-x, ]
  aoe3_test_temp <- aoe3_data[x, ]
  aoe3_model_boost10_temp <-  
    C5.0(playtime_above_median ~ ., data=aoe3_train_temp, trials = 10)
  aoe3_pred_boost10_temp <- predict(aoe3_model_boost10_temp, aoe3_test_temp)
  aoe3_actual_temp <- aoe3_test_temp$playtime_above_median
  kappa <- kappa2(data.frame(aoe3_actual_temp, aoe3_pred_boost10_temp))$value
  return(kappa)
})
